use crate::{
    core::FactorGraphBuilder,
    ising::{sum_product::SumProduct, IsingFactor, IsingMessagePassingType, IsingVariable},
};

fn exect_infinite_ising_1d_magnetization(
    coupling: f64,
    magnetic_field: f64,
    error: f64,
) -> (f64, f64) {
...
fn ising_1d_test() {
    ...
    let error = 1e-12f64;
    let mut fgb = FactorGraphBuilder::<
        IsingFactor<SumProduct>,
        IsingVariable<SumProduct>,
    >::new_with_variables(
        spins_number,
        spins_number - 1,
    );
    ...
    let _ = fg.run_message_passing_parallel(1000, error).unwrap();
    let marginals = fg.eval_marginals();
    let (exact_mid_spin, exact_bound_spin) =
        exect_infinite_ising_1d_magnetization(coupling, magnetic_field, error);
    let calculated_mid_spin = f64::tanh(marginals[spins_number / 2 + 1]);
    let calculated_bound_spin = f64::tanh(marginals[0]);
    assert!((exact_mid_spin - calculated_mid_spin).abs() < error * 10f64);
    assert!((exact_bound_spin - calculated_bound_spin).abs() < error * 10f64);
}
